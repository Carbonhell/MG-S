...
local f=GkEventTimerManager.Stop
...
local function _()n"MISSION_TIMER_UPDATE"end
...
function e.SetHelicopterDoorOpenTime(e)if not d(e)then
return
end
...
function e.SelectNextMissionHeliStartRoute(n,i,s)local t
if not s then
t=e.IsEmergencyMission(n)end
local s=TppDefine.NO_HELICOPTER_ROUTE_ENUM[tostring(n)]if not s then
...
function e._StartEmergencyMissionTimer(s,n,i)if not r(s)then
return
end
if not d(n)then
return
...
e.DisableInGameFlag()e.ResetEmegerncyMissionSetting()local s,i=vars.missionHeroicPoint,vars.missionOgrePoint
...
TppServerManager.AbortDefenseMotherBase()end
else
TppSave.VarRestoreOnMissionStart()end
e.SetHeroicAndOgrePointInSlot(s,i)e.RestartMission(n)end
...
function e.AbortMission(n)local u
local p
local m
local i
local d
...
local f
local O
local o,t,a=0,0,TppUI.FADE_SPEED.FADE_NORMALSPEED
local S
local M
if s(n)then
u=n.isNoFade
r=n.emergencyMissionId
l=n.nextMissionId
c=n.nextLayoutCode
T=n.nextClusterId
f=n.nextMissionStartRoute
d=n.isExecMissionClear
p=n.isNoSave
m=n.isInterrupt
O=n.isAlreadyGameOver
if n.delayTime then
o=n.delayTime
end
...
S=n.presentationFunction
i=n.isTitleMode
M=n.playRadio
end
if not e.CheckMissionState(d,true)then
return
end
...
mvars.mis_abortPresentationFunction=S
if i then
mvars.mis_abortIsTitleMode=i
end
mvars.mis_abortWithPlayRadio=M
mvars.mis_emergencyMissionCode=r
mvars.mis_nextMissionCodeForAbort=l
...
TppTerminal.ClearStaffNewIcon(i,a,s,t)TppEnemy.ClearDDParameter()if(not e.IsFOBMission(n)and not e.IsFreeMission(n))and not e.IsHelicopterSpace(n)then
...
function e.MissionGameEnd(n)local s=0
local t=0
...
e.FadeOutOnMissionGameEnd(t,i,"MissionGameEndFadeOutFinish")PlayRecord.RegistPlayRecord"MISSION_CLEAR"end
function e.FadeOutOnMissionGameEnd(n,i,s)if n==0 then
e._FadeOutOnMissionGameEnd(i,s)else
mvars.mis_missionGameEndFadeSpeed=i
mvars.mis_missionGameEndFadeId=s
t("Timer_FadeOutOnMissionGameEndStart",n)end
end
...
e.ResetNeedWaitMissionInitialize()end
function e.MissionFinalize(n)local o,r,t,i,a,l
if s(n)then
o=n.isNoFade
r=n.isExecGameOver
t=n.showLoadingTips
...
l=false
TppSave.VarSave(i,true)TppSave.SaveGameData(i,nil,nil,nil,true)end
if gvars.mis_nextMissionCodeForMissionClear~=c then
n=e.IsHelicopterSpace(vars.missionCode)t=e.IsFreeMission(vars.missionCode)a=e.IsHelicopterSpace(gvars.mis_nextMissionCodeForMissionClear)s=e.IsFreeMission(gvars.mis_nextMissionCodeForMissionClear)if mvars.heli_missionStartRoute then
...
TppTerminal.ClearStaffNewIcon(n,t,a,s)if n then
...
TppPlayer.SavePlayerCurrentAmmoCount()end
if i==10030 and TppSave.CanSaveMbMangementData(i)then
...
TppEnemy.ClearDDParameter()TppRevenge.OnMissionClearOrAbort(i)if gvars.solface_groupNumber>=4294967295 then
gvars.solface_groupNumber=0
else
gvars.solface_groupNumber=gvars.solface_groupNumber+1
end
...
local r=(vars.locationCode~=r)if not n then
TppTerminal.AddStaffsFromTempBuffer(nil,o)end
TppClock.SaveMissionStartClock()TppWeather.SaveMissionStartWeather()TppBuddyService.SetVarsMissionStart()TppBuddyService.BuddyMissionInit()TppRevenge.SaveMissionStartMineArea()TppMain.ReservePlayerLoadingPosition(TppDefine.MISSION_LOAD_TYPE.MISSION_FINALIZE,n,t,a,s,nil,r)TppWeather.OnEndMissionPrepareFunction()e.VarResetOnNewMission()if not e.IsFOBMission(vars.missionCode)then
local e=true
TppSave.VarSave(i,true)local n=false
do
n=true
end
if n and(not o)then
TppSave.SaveGameData(i,nil,nil,e,true)end
if mvars.mis_needSaveConfigOnNewMission then
TppSave.VarSaveConfig()TppSave.SaveConfigData(nil,nil,e)end
end
...
e.ResetMBFreeStartPositionToCommand()end
e.VarResetOnNewMission()if(vars.missionCode==10240)then
local e=TppPackList.GetLocationNameFormMissionCode(vars.missionCode)if e then
local e=TppDefine.LOCATION_ID[e]if e then
vars.locationCode=e
end
...
TppSave.VarSave()e.SetHeroicAndOgrePointInSlot(i,n)TppSave.SaveGameData(vars.missionCode)end
if TppRadio.playingBlackTelInfo then
mvars.mis_showLoadingTipsOnMissionFinalize=false
end
e.RequestLoad(vars.missionCode,i,{showLoadingTips=mvars.mis_showLoadingTipsOnMissionFinalize,waitOnLoadingTipsEnd=l,ignoreMtbsLoadLocationForce=mvars.mis_missionFinalizeIgnoreMtbsLoadLocationForce})end
...
end}},UI={{msg="EndTelopCast",func=function()if mvars.f30050_demoName=="NuclearEliminationCeremony"then
return
end
...
TppUiCommand.RemovedAllUserMarker()end
end},{msg="Finish",sender=l,func=function()if(mvars.mis_isAlertOutOfMissionArea==false)then
return
end
...
TppSound.ClearOnDecendingLandingZoneJingleFlag()end},{msg="DescendToLandingZone",func=function()local e=e.CheckMissionClearOnOutOfMissionArea()local n=svars.mis_canMissionClear
if e or n then
TppSound.PostJingleOnDecendingLandingZone()else
TppSound.PostJingleOnDecendingLandingZoneWithOutCanMissionClear()end
end},{msg="StartedPullingOut",func=function()t("Timer_RemoveUserMarker",1)end},{msg="LostControl",func=function(i,e,n)local i=GameObject.GetTypeIndex(i)if i~=TppGameObject.GAME_OBJECT_TYPE_HELI2 then
return
end
if e==o"Start"then
...
if e==o"End"then
local e=TppSupportRequest.GetCrashRescueHeliGmpCost()TppTerminal.UpdateGMP{gmp=-e,gmpCostType=TppDefine.GMP_COST_TYPE.DESTROY_SUPPORT_HELI}if Tpp.IsPlayer(n)then
...
local T=o"FallDeath"local u=o"Suicide"function e.OnPlayerDead(s,n)if not TppNetworkUtil.IsHost()then
return
end
local i=e.IsFOBMission(vars.missionCode)if(not i)or TppPlayer.IsSneakPlayerInFOB(s)then
if n==T then
...
n=mvars.heli_missionStartRoute
end
local s=TppDefine.STORY_MISSION_CLUSTER_ID[gvars.mis_nextMissionCodeForMissionClear]if i then
s=i
end
e.ReserveMissionClear{missionClearType=TppDefine.MISSION_CLEAR_TYPE.FROM_HELISPACE,nextMissionId=gvars.mis_nextMissionCodeForMissionClear,nextHeliRoute=n,nextClusterId=s}else
gvars.usingNormalMissionSlot=false
...
function e.OnAbortMissionPreparation()e.SetNextMissionCodeForMissionClear(c)gvars.heli_missionStartRoute=0
end
...
if t and i then
e.SetMissionObjectives(t,i,o)end
...
e.MessageHandler={OnMessage=function(i,s,n,a,o,t)e.OnMessageWhileLoading(i,s,n,a,o,t)end}GameMessage.SetMessageHandler(e.MessageHandler,{"UI","Radio","Video","Network","Nt"})end
...
function e.ExecuteSystemCallback(s,n)local e=e.systemCallbacks[s]if i(e)then
return e(n)end
end
...
local r=TppDemo.IsNotPlayable()local n=false
...
local function u()if p"Timer_OutsideOfHotZoneCount"then
f"Timer_OutsideOfHotZoneCount"end
end
...
local s=svars
local i=e.GetMissionName()if n.mis_needSetCanMissionClear then
e._SetCanMissionClear()end
if n.mis_missionStateIsNotInGame then
return
end
local T,S,u,c=e.GetSyncMissionStatus()local m=n.mis_isAlertOutOfMissionArea
local o=n.mis_isOutsideOfMissionArea
local d=n.mis_isOutsideOfHotZone
local r=s.mis_canMissionClear
if T and S then
TppMain.DisableGameStatus()HighSpeedCamera.RequestToCancel()e.EstablishedMissionClear(s.mis_missionClearType)elseif u and c then
...
elseif r then
e.UpdateAtCanMissionClear(d,o)else
if o then
...
local n=false
if a then
t=e.mis_isDefiniteMissionClear and m"mis_isDefiniteMissionClear"s=m"mis_missionClearType"i=e.mis_isDefiniteGameOver and m"mis_isDefiniteGameOver"n=m"mis_gameOverType"else
if r then
...
local t=Vector3(0,-.75,1.98)local o=Vector3(e[1],e[2],e[3])local e=-Quat.RotationY(TppMath.DegreeToRadian(n)):Rotate(t)i=e+o
s=n
end
end
end
...
local i=v()local s=R()local n=not a(vars.playerVehicleGameObjectId)if o then
if s and n then
u()e.ReserveMissionClearOnOutOfHotZone()end
else
if(i and s)and n then
if not p"Timer_OutsideOfHotZoneCount"then
t("Timer_OutsideOfHotZoneCount",A)end
else
if not i then
...
a=TppPlayer.PlayMissionAbortCamera
i=true
end
end
if TppLocation.IsAfghan()then
e.AbortMission{nextMissionId=TppDefine.SYS_MISSION_ID.AFGH_FREE,isNoSave=n,fadeDelayTime=t,fadeSpeed=o,presentationFunction=a,playRadio=i}elseif TppLocation.IsMiddleAfrica()then
e.AbortMission{nextMissionId=TppDefine.SYS_MISSION_ID.MAFR_FREE,isNoSave=n,fadeDelayTime=t,fadeSpeed=o,presentationFunction=a,playRadio=i}else
e.AbortMission{nextMissionId=TppDefine.SYS_MISSION_ID.AFGH_FREE,isNoSave=n,fadeDelayTime=t,fadeSpeed=o,presentationFunction=a,playRadio=i}end
end
...
function e._SetMissionOrderBoxPosition(n)local e,i=e.GetOrderBoxLocator(n)if e then
local n=Vector3(0,-.75,1.98)local e=Vector3(e[1],e[2],e[3])local n=-Quat.RotationY(TppMath.DegreeToRadian(i)):Rotate(n)local e=n+e
local n=TppMath.Vector3toTable(e)local e=i
TppPlayer.SetInitialPosition(n,e)TppPlayer.SetMissionStartPosition(n,e)return true
end
end
...
function e.GetOrderBoxLocator(e)if not r(e)then
return
end
return Tpp.GetLocator("OrderBoxIdentifier",e)end
function e.GetOrderBoxLocatorByTransform(e)if not r(e)then
end
...
TppSave.EraseAllGameDataSaveRequest()TppSave.VarSave()if DebugMenu then
...
if#mvars.mis_missionObjectiveEnum>C then
return
end
end
...
function e._ShowObjective(e,i)if e.packLabel then
...
TppUI.EnableMissionTask(e.missionTask,i)end
if e.spySearch then
...
function e.DisableChildrenObjective(i)local n
...
if not n then
TppSave.SaveGameData(nil,nil,nil,nil,true)e.CreateMbSaveCoroutine()end
end
...
TppPlayer.ResetStealthAssistCount()TppSave.ReserveVarRestoreForMissionStart()TppResult.ClearNewestPlayStyleHistory()e.SetNextMissionCodeForMissionClear(c)e.ResetMissionClearState()end
...
s=gvars.mis_nextClusterIdForEmergency
end
e.ReserveMissionClear{missionClearType=TppDefine.MISSION_CLEAR_TYPE.FROM_HELISPACE,nextMissionId=i,nextHeliRoute=t,nextLayoutCode=n,nextClusterId=s}end
function e.RequestLoad(e,i,n)if not mvars then
return
end
if gvars.isLoadedInitMissionOnSignInUserChanged then
return
end
TppMain.EnablePause()mvars.mis_loadRequest={nextMission=e,currentMission=i,options=n}end
function e.LoadWithChunkCheck()local i,n,s=mvars.mis_loadRequest.nextMission,mvars.mis_loadRequest.currentMission,mvars.mis_loadRequest.options
if DebugText then
local e=DebugText.NewContext()DebugText.Print(e,{.5,.5,1},"Requested load : nextMission = "..(tostring(i)..(", currentMission = "..tostring(n))))end
local t=Tpp.GetChunkIndex(vars.locationCode)if e.IsChunkLoading(t)then
return
end
e.Load(i,n,s)mvars.mis_loadRequest=nil
end
...
local t=TppPackList.GetLocationNameFormMissionCode(n)local o=TppPackList.GetLocationNameFormMissionCode(i)local s
...
function e.ResetQuietEquipIfUndevelop()if vars.buddyQuietEquipType==4 then
if not TppMotherBaseManagement.IsEquipDevelopedFromDevelopID{equipDevelopID=6094}then
TppBuddyService.SetVarsQuietWeaponType(0)end
end
if not Tpp.IsMaster()then
if vars.buddyQuietEquipType>4 then
Fox.Hungup"Unkown quiet equip type! please check need to check developed"end
end
end
...
function e.SetPlayRecordClearInfo()local n,e=TppStory.CalcAllMissionClearedCount()TppUiCommand.SetPlayRecordClearInfo{recordId="MissionClear",clearCount=n,allCount=e}local e,n=TppStory.CalcAllMissionTaskCompletedCount()TppUiCommand.SetPlayRecordClearInfo{recordId="MissionTaskClear",clearCount=e,allCount=n}local e,n=TppQuest.CalcQuestClearedCount()TppUiCommand.SetPlayRecordClearInfo{recordId="SideOpsClear",clearCount=e,allCount=n}end
...
function e.SetHeroicAndOgrePointInSlot(n,e)TppScriptVars.SetVarValueInSlot(TppDefine.SAVE_SLOT.MISSION_START,"vars","missionHeroicPoint",n)TppScriptVars.SetVarValueInSlot(TppDefine.SAVE_SLOT.CHECK_POINT_RESTARTABLE,"vars","missionHeroicPoint",n)TppScriptVars.SetVarValueInSlot(TppDefine.SAVE_SLOT.MISSION_START,"vars","missionOgrePoint",e)TppScriptVars.SetVarValueInSlot(TppDefine.SAVE_SLOT.CHECK_POINT_RESTARTABLE,"vars","missionOgrePoint",e)end
