...
function e.StartTitle(a)TppSystemLua.UseAiSystem(true)TppSimpleGameSequenceSystem.Start()local function t()TppSave.CopyGameDataFromSavingSlot()e.InitializeForNewMission{}gvars.elapsedTimeSinceLastPlay=TppScriptVars.GetElapsedTimeSinceLastPlay()gvars.elapsedTimeSinceLastUseChickCap=gvars.elapsedTimeSinceLastUseChickCap+gvars.elapsedTimeSinceLastPlay
...
function e.InitializeForNewMission(e)TppSave.VarRestoreOnMissionStart()if(Tpp.IsQARelease()or nil)then
TppDebug.DEBUG_RestoreSVars()end
TppStory.DisableMissionNewOpenFlag(vars.missionCode)TppClock.RestoreMissionStartClock()if e.sequence and e.sequence.MISSION_START_INITIAL_WEATHER then
...
vars.initSupportWeapons[n]=a
vars.ammoStockIds[e]=i
vars.ammoStockCounts[e]=t
else
vars.initWeapons[e]=a
vars.ammoStockIds[e]=i
vars.ammoStockCounts[e]=t
...
function e.InitializeAllPlatformForNewGame()local i=0
local a=1
local t={"Command","Combat","Develop","BaseDev","Support","Spy","Medical"}local e={"MotherBase","Fob1","Fob2","Fob3","Fob4"}for a,e in ipairs(e)do
for t,a in ipairs(t)do
TppMotherBaseManagement.SetClusterSvars{base=e,category=a,grade=i,buildStatus="Completed",timeMinute=0,isNew=false}end
end
TppMotherBaseManagement.SetClusterSvars{base="MotherBase",category="Command",grade=a,buildStatus="Completed",timeMinute=0,isNew=true}end
