...
function e.SaveGameData(t,r,i,a,n)if a then
e.ReserveNextMissionStartSave(e.GetGameSaveFileName(),n)else
local a=e.GetSaveGameDataQueue(t,r,i,n)e.EnqueueSave(a)end
e.CheckAndSavePersonalData(a)end
function e.GetSaveGameDataQueue(r,i,t,n)local a=e.GetGameSaveFileName()local a=e.GetIntializedCompositSlotSaveQueue(a,i,t,n)a=e._SaveGlobalData(a)a=e._SaveMissionData(a)a=e._SaveMissionRestartableData(a)a=e._SaveRetryData(a)a=e._SaveMbManagementData(a,r)a=e._SaveQuestData(a)return a
end
...
function e.MakeNewGameSaveData(t)TppVarInit.InitializeOnNewGameAtFirstTime()TppVarInit.InitializeOnNewGame()if t then
TppTerminal.AcquirePrivilegeInTitleScreen()end
e.VarSave(vars.missionCode,true)e.VarSaveOnRetry()local n,a=e.GetSaveGameDataQueue(vars.missionCode)if gvars.permitGameSave then
n=e.GetSaveGameDataQueue(vars.missionCode)a=e.DoSave(n,true)end
if t then
e.CheckAndSavePersonalData()end
return a
end
function e.SaveImportedGameData()local a,n=e.GetSaveGameDataQueue(vars.missionCode)if gvars.permitGameSave then
a=e.GetSaveGameDataQueue(vars.missionCode)n=e.DoSave(a,true)end
return n
end
if DebugMenu then
function e.MakeRescueMillerSaveData()TppVarInit.InitializeOnNewGameAtFirstTime()TppVarInit.InitializeOnNewGame()vars.locationCode=TppDefine.LOCATION_ID.AFGH
vars.missionCode=10020
gvars.str_storySequence=TppDefine.STORY_SEQUENCE.CLEARD_ESCAPE_THE_HOSPITAL
TppStory.UpdateMissionCleardFlag(10010)TppStory.PermitMissionOpen(10020)TppStory.MissionOpen(10020)local a={{slot=TppDefine.WEAPONSLOT.PRIMARY_HIP,equip=TppEquip.EQP_WP_30101,bulletId=TppEquip.BL_Rf556x45mm,ammoMax=30,ammo=240},{slot=TppDefine.WEAPONSLOT.SECONDARY,equip=TppEquip.EQP_WP_10101,bulletId=TppEquip.BL_HgTranq,ammoMax=7,ammo=21},{slot=TppDefine.WEAPONSLOT.SUPPORT_0,equip=TppEquip.EQP_SWP_Grenade,bulletId=TppEquip.BL_SWP_Grenade,ammoMax=8,ammo=8},{slot=TppDefine.WEAPONSLOT.SUPPORT_1,equip=TppEquip.EQP_SWP_Magazine,bulletId=TppEquip.BL_SWP_Magazine,ammoMax=-1,ammo=-1}}TppVarInit.SetInitPlayerWeapons(a)local a={TppEquip.EQP_None,TppEquip.EQP_None,TppEquip.EQP_None,TppEquip.EQP_None}TppVarInit.SetInitPlayerItems(a)e.VarSave(vars.missionCode,true)e.VarSaveOnRetry()e.SaveGameData(vars.missionCode)end
end
function e.GetIntializedCompositSlotSaveQueue(t,n,a,e)return{fileName=t,needIcon=n,doSaveFunc=a,isCheckPoint=e}end
...
function e.EnqueueSave(a,t,i,r,S)if a==nil then
return
end
...
e.saveQueueList[e.saveQueueDepth]=e.MakeNewSaveQueue(a,t,i,r,S)end
end
function e.MakeNewSaveQueue(t,a,S,r,i,n)local e={}e.slot=t
e.savingSlot=a
e.category=S
e.fileName=r
e.needIcon=i
...
function e.DoSave(a,n)local s=true
if n then
s=false
end
local n
...
n=a.category
if n then
p=e.GetSaveFileVersion(n)t=a.fileName
r=a.needIcon
i=a.doSaveFunc
...
if i then
i()end
local e=TppScriptVars.WriteSlotToFile(a.savingSlot,t,r)if s then
gvars.sav_SaveResultCheckFileName=Fox.StrCode32(t)if S then
...
if(gvars.sav_SaveResultCheckFileName~=0)then
local n=true
local a=TppScriptVars.GetLastResult()local a,t=e.GetSaveResultErrorMessage(a)if a then
n=false
TppUiCommand.ShowErrorPopup(a,t)end
local e=e.SAVE_RESULT_FUNCTION[gvars.sav_SaveResultCheckFileName]if e then
e(n)end
gvars.sav_SaveResultCheckFileName=0
...
function e.OnMessage(t,n,a,i,S,r,p)Tpp.DoMessage(e.messageExecTable,TppMission.CheckMessageOption,t,n,a,i,S,r,p)end
...
function e.SaveVarsToSlot(n,t,a)local e=e.GetSaveFileVersion(a)TppScriptVars.SaveVarsToSlot(n,t,a,e)end
...
function e.GetGameSaveFileName()if(Tpp.IsMaster()or Tpp.IsQARelease())then
...
function e.CopyGameDataFromSavingSlot()for e,a in ipairs(n)do
...
function e.CheckSlotVersion(a,n,i)local t=e.GetSaveFileVersion(a)local e=TppDefine.SAVE_FILE_INFO[a].slot
if i then
...
if TppRevenge.QARELEASE_DEBUG_InitDebugMenuRevengePoint then
TppRevenge.QARELEASE_DEBUG_InitDebugMenuRevengePoint()end
end
end
...
DebugMenu.AddDebugMenu(" SaveData","GoCyprEndEntry","bool",mvars.qaDebug,"sav_cyprEndEntry")mvars.qaDebug.sav_makeRescueMillerSaveData=false
DebugMenu.AddDebugMenu(" SaveData","MakeRescueMillerSaveData","bool",mvars.qaDebug,"sav_makeRescueMillerSaveData")mvars.qaDebug.sav_makeFullOpenSaveData=false
...
DebugMenu.AddDebugMenu(" SaveData","DeletePersonalDataFile","bool",mvars.qaDebug,"sav_deletePersonalDataFile")mvars.qaDebug.sav_deleteMgoFile=false
DebugMenu.AddDebugMenu(" SaveData","DeleteMGOFile","bool",mvars.qaDebug,"sav_deleteMgoFile")mvars.qaDebug.sav_makeConfigFile=false
...
e.MakeRescueMillerSaveData()end
...
if a.qaDebug.sav_deleteMgoFile then
a.qaDebug.sav_deleteMgoFile=false
e.DeleteMGOSaveFile()end
