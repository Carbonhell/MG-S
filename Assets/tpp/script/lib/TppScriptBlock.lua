...
local c=mvars.sbl_scriptBlockPackList[t]if not c then
return
end
local t=svars.sbl_scriptBlockPack[e]if t==0 then
return
end
...
function e.SetScriptBlockState(e,c)local e=e or r()if e==ScriptBlock.SCRIPT_BLOCK_ID_INVALID then
return false
end
if e>=t then
return false
...
function e.Load(c,r,i,t)local n=true
if t==false then
n=false
end
local t=ScriptBlock.GetScriptBlockId(c)if t~=ScriptBlock.SCRIPT_BLOCK_ID_INVALID then
e.SaveScriptBlockId(c,t)local c=e.FindPackList(c,r)if c then
svars.sbl_scriptBlockPack[t]=l(r)if n then
...
function e.ResolveSavedScriptBlockInfo(t)local l=svars.sbl_scriptBlockName[t]local r=svars.sbl_scriptBlockPack[t]if l==0 then
return
end
if r==0 then
return
...
local n=ScriptBlock.GetScriptBlockId(c)if n~=ScriptBlock.SCRIPT_BLOCK_ID_INVALID then
mvars.sbl_currentScriptBlockPackInfo[n]={scriptBlockNameHash=l,packListKeyHash=r}else
