...
function e.OpenChildSoldier_1()local t=e.IsCleard"mtbs_q99050"local e=e.IsNowOccurringElapsed()local n=TppStory.IsNowOccurringElapsedMission(TppDefine.ELAPSED_MISSION_EVENT.STORY_SEQUENCE)local a=TppStory.IsMissionCleard(10093)return((n or e)and t)and a
end
...
function t.mtbs_q99011()local t=TppBuddy2BlockController.DidObtainBuddyType(BuddyType.QUIET)local n=TppStory.IsMissionCleard(10086)local e=TppStory.IsNowOccurringElapsedMission(TppDefine.ELAPSED_MISSION_EVENT.QUIET_VISIT_MISSION)return(t and n)and e
end
...
local s=t.questName
local o=e.IsActiveOnMBTerminal(t)local a=e.IsCleard(s)if t and(o or a)then
t.index=i
t.isActive=o
t.isCleard=a
...
function e.IsActiveOnMBTerminal(n)local t=n.questName
...
local l={}function l.mtbs_wait_quiet()return TppStory.CanArrivalQuietInMB()end
function l.Mtbs_child_dog()local t=TppBuddyService.CanSortieBuddyType(BuddyType.DOG)local e=not TppStory.IsNowOccurringElapsedMission(TppDefine.ELAPSED_MISSION_EVENT.D_DOG_GO_WITH_ME)return(not t)and e
end
function l.Mtbs_ddog_walking()local e=TppBuddyService.IsDeadBuddyType(BuddyType.DOG)return(not e)end
function l.cliffTown_q99080()local e=TppMotherBaseManagement.IsExistStaff{uniqueTypeId=110}local t=TppStory.IsMissionCleard(10240)return e or t
end
...
function e.RegisterQuestSystemCallbacks(t)if not o(t)then
return
end
mvars.qst_systemCallbacks=mvars.qst_systemCallbacks or{}local function n(t,e)if r(t[e])then
mvars.qst_systemCallbacks[e]=t[e]end
end
local e={"OnActivate","OnOutOfAcitveArea","OnDeactivate","OnTerminate"}for a=1,#e do
n(t,e[a])end
end
...
local t=e.GetQuestStepTable(gvars.qst_currentQuestStepNumber)local e=t.OnLeave
if r(e)then
e(t)end
end
...
TppMission.SetPlayRecordClearInfo()TppChallengeTask.RequestUpdate"SIDEOPS"TppUiCommand.SetSideOpsListUpdate()for n,e in ipairs(m)do
if t==e then
TppMotherBaseManagement.SetLockedTanFlag{locked=false}return
end
end
end
function e.GoToMBAfterClear(e)if not Tpp.IsNotAlert()then
return
end
...
local t=TppStory.GetForceMBDemoNameOrRadioList("clearSideOpsForceMBDemo",{clearSideOpsName=e})if t then
TppMission.ReserveMissionClear{nextMissionId=TppDefine.SYS_MISSION_ID.MTBS_FREE,missionClearType=TppDefine.MISSION_CLEAR_TYPE.FORCE_GO_TO_MB_ON_SIDE_OPS_CLEAR}TppDemo.SetNextMBDemo(t)mvars.qst_currentClearQuestName=e
end
end
...
local i,a=TppEnemy.GetQuestCount()local n,o=TppGimmick.GetQuestShootingPracticeCount()if i>0 and a>1 then
e.ShowAnnounceLog(s.UPDATE,t,i,a)elseif n>0 and o>1 then
e.UpdateShootingPracticeUi()e.ShowAnnounceLog(s.UPDATE,t,n,o)end
end
...
function e.Messages()local t=e.DeactiveQuestAreaTrapMessages()return A{Block={{msg="StageBlockCurrentSmallBlockIndexUpdated",func=e.OnUpdateSmallBlockIndex}},UI={{msg="EndFadeOut",sender="FadeOutOnOutOfMissionArea",func=function()mvars.qst_blockStateRequest=i.DEACTIVATE
TppUI.FadeIn(TppUI.FADE_SPEED.FADE_NORMALSPEED)end},{msg="QuestAreaAnnounceText",func=function(t)e.OnQuestAreaAnnounceText(t)end}},Marker={{msg="ChangeToEnable",func=function(s,n,t,a)e._ChangeToEnable(s,n,t,a)end}},Timer={{msg="Finish",sender="TimerShootingPracticeStart",func=function()e.StartShootingPractice()end,option={isExecMissionPrepare=true,isExecMissionClear=true}},{msg="Finish",sender="TimerShootingPracticeEnd",func=function()e.OnQuestShootingTimerEnd()end,option={isExecMissionPrepare=true,isExecMissionClear=true}},{msg="Finish",sender="TimerShootingPracticeRetryConfirm",func=function()TppGimmick.SetQuestShootingPracticeTargetInvisible()end,option={isExecMissionPrepare=true,isExecMissionClear=true}}},Trap=t}end
function e.OnMessage(i,a,s,r,t,n,o)Tpp.DoMessage(e.messageExecTable,TppMission.CheckMessageOption,i,a,s,r,t,n,o)local l=mvars.qst_questScriptBlockMessageExecTable
if l then
local e=o
local o
Tpp.DoMessage(l,TppMission.CheckMessageOption,i,a,s,r,t,n,e)end
...
local o=o
local l
Tpp.DoMessage(e,TppMission.CheckMessageOption,i,a,s,r,t,n,o)end
end
end
end
...
if not S(e.name)then
return
end
if not S(e.invokeStepName)then
return
end
end
...
local t=t.name
if D(t)==gvars.qst_currentQuestName then
e.SetCurrentQuestName(t)end
end
end
return mvars.qst_questList
end
function e.RegisterQuestPackList(t,r)if not o(t)then
return
end
r=r or _
local o=TppLocation.IsMotherBase()local a={}for t,n in pairs(t)do
a[t]={}for s,n in pairs(n)do
if type(n)=="number"then
table.insert(a[t],n)elseif s=="faceIdList"then
local e=TppSoldierFace.GetFaceFpkFileCodeList{face=n,useHair=o}if e~=nil then
for n,e in ipairs(e)do
table.insert(a[t],e)end
end
elseif s=="bodyIdList"then
local e=TppSoldierFace.GetBodyFpkFileCodeList{body=n}if e~=nil then
for n,e in ipairs(e)do
table.insert(a[t],e)end
end
elseif s=="randomFaceList"then
if e.IsRandomFaceQuestName(t)then
if n.race and n.gender then
if TppMission.IsMissionStart()then
local a=(math.random(0,65535)*65536)+math.random(1,65535)local a=TppSoldierFace.CreateFaceTable{race=n.race,gender=n.gender,needCount=1,maxUsedFovaCount=1,seed=a}if a~=nil then
for a,n in ipairs(a)do
e.SetRandomFaceId(t,n)end
else
if n.gender==TppDefine.QUEST_GENDER_TYPE.MAN then
e.SetRandomFaceId(t,TppDefine.QUEST_FACE_ID_LIST.DEFAULT_MAN)elseif n.gender==TppDefine.QUEST_GENDER_TYPE.WOMAN then
e.SetRandomFaceId(t,TppDefine.QUEST_FACE_ID_LIST.DEFAULT_WOMAN)end
end
end
local o=e.GetRandomFaceId(t)local i={o}local s=TppSoldierFace.GetFaceFpkFileCodeList{face=i}if s==nil then
if n.gender==TppDefine.QUEST_GENDER_TYPE.MAN then
e.SetRandomFaceId(t,TppDefine.QUEST_FACE_ID_LIST.DEFAULT_MAN)elseif n.gender==TppDefine.QUEST_GENDER_TYPE.WOMAN then
e.SetRandomFaceId(t,TppDefine.QUEST_FACE_ID_LIST.DEFAULT_WOMAN)end
o=e.GetRandomFaceId(t)i={o}s=TppSoldierFace.GetFaceFpkFileCodeList{face=i}end
if s~=nil then
...
table.insert(a[t],n)end
end
end
TppScriptBlock.RegisterCommonBlockPackList(r,a)end
function e.SetDefaultQuestBlock()mvars.qst_blockName=_
end
...
local n,a=Tpp.GetCurrentStageSmallBlockIndex()e.UpdateQuestBlockStateAtNotLoaded(n,a,t)end
function e.InitializeQuestActiveStatus(n)local t=e.GetQuestBlockState()if t==nil then
return
end
...
if t<ScriptBlock.SCRIPT_BLOCK_STATE_INACTIVE or not e._CanActivateQuest()then
mvars.qst_requestInitializeQuestActiveStatus=true
mvars.qst_requestInitializeQuestActiveCluster=n
return
end
local t=e.GetCurrentQuestTable()if t==nil then
return
end
local s,a=Tpp.GetCurrentStageSmallBlockIndex()if e.IsInsideArea("activeArea",t,s,a,n)then
e.ActivateCurrentQuestBlock()end
if not e.IsInvoking()then
if e.IsInsideArea("invokeArea",t,s,a,n)then
e.Invoke()end
else
...
local t=DebugText.NewContext()if n.debug.showCurrentQuest then
a(t,"")a(t,{.5,.5,1},"LuaQuest showCurrentQuest")a(t,"Current Area Name : "..tostring(e.GetCurrentAreaName()))a(t,"Current Quest Name : "..tostring(e.GetCurrentQuestName()))local o=e.GetQuestBlockState()local s={}s[ScriptBlock.SCRIPT_BLOCK_STATE_EMPTY]="EMPTY"s[ScriptBlock.SCRIPT_BLOCK_STATE_PROCESSING]="PROCESSING"s[ScriptBlock.SCRIPT_BLOCK_STATE_INACTIVE]="INACTIVE"s[ScriptBlock.SCRIPT_BLOCK_STATE_ACTIVE]="ACTIVE"a(t,"Quest block state : "..tostring(s[o]))a(t,"gvars.qst_currentQuestName : "..tostring(gvars.qst_currentQuestName))a(t,"gvars.qst_currentQuestStepNumber : "..tostring(gvars.qst_currentQuestStepNumber))do
local i={0,1,0}local r={1,0,0}local s="OK"local i=i
if not e.GetCurrentAreaName()or o<ScriptBlock.SCRIPT_BLOCK_STATE_INACTIVE then
s="---"elseif not n.qst_questStepTable then
s="No register quest step table! Please Check quest script!"i=r
end
a(t,i,"scriptStatus : "..s)end
end
...
local function a(t,n,e)local a=DebugText.Print
a(t,string.format("%s = { %03d, %03d, %03d, %03d }",n,e[1],e[2],e[3],e[4]))end
local e={"loadArea","activeArea","invokeArea"}for n=1,#e do
local e=e[n]local n=s[e]if n then
a(t,e,n)end
end
end
...
local i=n.debug.selectQuestIndex
if e>0 and i>0 then
local s=n.qst_questList[e]if s then
local o=s.infoList[i]if o then
local e=TppDefine.QUEST_INDEX[o.name]gvars.qst_questOpenFlag[e]=n.debug.updateOpenFlagSelectQuest
...
for t,e in pairs(s.infoList)do
...
a(t,"No define quest. index: "..tostring(i))end
end
end
...
local r=ScriptBlock.SCRIPT_BLOCK_STATE_INACTIVE
...
e.UpdateQuestBlockStateAtNotLoaded(n,a,s)elseif(t==r)then
e.UpdateQuestBlockStateAtInactive(n,a,s)elseif(t==l)then
...
function e.UpdateQuestBlockStateAtNotLoaded(s,a,t)if not mvars.qst_questList then
return
end
local n=e.GetCurrentQuestName()local t=e.SearchQuestFromAllSpecifiedArea("loadArea",s,a,t)if t==nil then
e.UnloadCurrentQuestBlock()e.ClearCurrentQuestName()e.ResetQuestStatus()end
if n then
local a=e.GetQuestBlockState()local s=ScriptBlock.SCRIPT_BLOCK_STATE_EMPTY
if t then
if(a==s or n~=t)then
...
function e.UpdateQuestBlockStateAtInactive(n,a)local t=e.GetCurrentQuestTable()if not e.IsInsideArea("loadArea",t,n,a)then
e.UnloadCurrentQuestBlock()return
end
if e.IsInsideArea("activeArea",t,n,a)then
if not e.IsInvoking()then
if e.IsInsideArea("invokeArea",t,n,a)then
...
function e.UpdateQuestBlockStateAtActive(a,n)local t=e.GetCurrentQuestTable()if not e.IsInsideArea("activeArea",t,a,n)then
...
function e.QuestBlockOnUpdate()local e=e
local a=e.GetQuestBlockState()if a==nil then
return
end
local n=ScriptBlock
local t=mvars
local s=t.qst_lastQuestBlockState
local o=n.SCRIPT_BLOCK_STATE_INACTIVE
local n=n.SCRIPT_BLOCK_STATE_ACTIVE
if t.qst_requestInitializeQuestActiveStatus then
e.InitializeQuestActiveStatus(t.qst_requestInitializeQuestActiveCluster)return
end
if a==o then
if s==n then
e._DoDeactivate()end
if t.qst_blockStateRequest==i.ACTIVATE then
if e._CanActivateQuest()then
e.ActivateCurrentQuestBlock()e.ClearBlockStateRequest()end
end
t.qst_lastInactiveToActive=false
elseif a==n then
...
local n
if e.IsInvoking()then
n=e.GetQuestStepTable(gvars.qst_currentQuestStepNumber)end
if t.qst_lastInactiveToActive then
t.qst_lastInactiveToActive=false
t.qst_deactivated=false
e.ExecuteSystemCallback"OnActivate"t.qst_allocated=true
if t.qst_invokeReserveOnActivate then
t.qst_invokeReserveOnActivate=false
e.Invoke()n=e.GetQuestStepTable(gvars.qst_currentQuestStepNumber)end
if n and r(n.OnEnter)then
n.OnEnter(n)end
end
if(not s)or s<=o then
t.qst_lastInactiveToActive=true
end
if n and r(n.OnUpdate)then
n.OnUpdate(n)end
if t.qst_blockStateRequest==i.DEACTIVATE then
e.DeactivateCurrentQuestBlock()e.ClearBlockStateRequest()end
else
t.qst_lastInactiveToActive=false
e.ClearBlockStateRequest()end
t.qst_lastQuestBlockState=a
end
...
gvars.qst_currentQuestName=T
end
function e.ResetQuestStatus()gvars.qst_currentQuestName=T
gvars.qst_currentQuestStepNumber=c
end
...
function e.SearchQuestFromAllSpecifiedArea(n,s,a,o)local t=#mvars.qst_questList
for t=1,t do
local t=mvars.qst_questList[t]if e.IsInsideArea(n,t,s,a,o)then
...
function e.IsInsideArea(o,e,s,a,n)do
...
local t=e.areaName
local e=e[o]if e==nil then
return
end
return Tpp.CheckBlockArea(e,s,a)end
end
function e.GetCurrentQuestTable()return mvars.qst_currentQuestTable
end
function e.GetQuestTable(n)local e=#mvars.qst_questList
for e=1,e do
local e=mvars.qst_questList[e]for a,t in ipairs(e.infoList)do
if t.name==n then
return e,t
end
end
...
local n=nil
local n={}local o={}local s={}local n={}for t,a in ipairs(t.infoList)do
local t=a.name
local i=TppDefine.QUEST_INDEX[t]if i then
gvars.qst_questActiveFlag[i]=false
local i=l[t]if e.IsOpen(t)and(not i or i())then
...
TppUiCommand.SetSideOpsListUpdate()for t,e in ipairs(m)do
...
function e.CanActiveQuestInMission(e,t)if(not TppMission.IsStoryMission(e))then
return true
else
...
local n,a=Tpp.GetCurrentStageSmallBlockIndex()if not e.IsInsideArea("loadArea",t,n,a)then
return true
end
end
end
...
local e=l[t]if(e==nil)or e()then
return
end
end
end
...
local e=l[t.name]if e and(not e())then
...
function e.CalcQuestClearedCount()local t=0
local e=0
...
if e then
TppTrophy.Unlock(16,TppHero.MINE_QUEST_ALL_CLEAR.heroicPoint,TppHero.MINE_QUEST_ALL_CLEAR.ogrePoint)end
end
...
function e.GetElapsedCount()return gvars.qst_elapseCount
end
...
function e.GetClearKeyItem(n)for e,t in pairs(q)do
if e==n then
TppTerminal.AcquireKeyItem{dataBaseId=t,isShowAnnounceLog=true}for n,t in pairs(N)do
if e==n then
TppUI.ShowAnnounceLog("quest_get_photo",t)end
end
end
end
end
function e.GetClearEmblem(e)local e=p[e]if e then
for t,e in ipairs(e)do
TppEmblem.Add(e,false,true)end
end
end
function e.GetClearCassette(t)local a={"outland_q20913","lab_q20914","tent_q20910","sovietBase_q20912","fort_q20911"}local n={{"tp_m_10160_06"},{"tp_m_10160_07"},{"tp_m_10160_08"},{"tp_m_10160_09","tp_m_10160_10"}}if(((t=="outland_q20913"or t=="lab_q20914")or t=="tent_q20910")or t=="sovietBase_q20912")or t=="fort_q20911"then
local t=0
for a,n in ipairs(a)do
...
function e.ShowAnnounceLog(n,t,u,l)if not n then
return
end
if n==s.OPEN then
...
local o=e.GetQuestNameLangId(t)if o~=false then
local n=f[t]local s,r=TppEnemy.GetQuestCount()local a,i=TppGimmick.GetQuestShootingPracticeCount()if n then
if s>1 then
TppUI.ShowAnnounceLog(n,s,r)elseif a>1 then
TppUI.ShowAnnounceLog(n,a,i)end
end
TppUI.ShowAnnounceLog"quest_list_update"TppUI.ShowAnnounceLog("quest_complete",o)local e=e.GetQuestNameId(t)if(e~=false)and(e~="q99012")then
...
local e=f[t]if e then
TppUI.ShowAnnounceLog(e,u,l)end
end
end
...
function e.DeactiveQuestAreaTrapMessages()local a={}local t={}local n=TppMission.GetMissionID()if n==30010 then
t=h
elseif n==30020 then
t=C
else
...
local e=e.GetTrapName(t)local e={msg="Exit",sender=e,func=function(e,e)TppEnemy.CheckDeactiveQuestAreaForceFulton()end}table.insert(a,e)end
return a
end
...
local a=TppEnemy.IsQuestTarget(t)local n=TppGimmick.IsQuestTarget(t)local t=TppAnimal.IsQuestTarget(t)if(a or n)or t then
...
function e.OnFinishShootingPractice(t,n)if t or n then
e.ProcessFinishShootingPractice(t,n)end
...
function e.ShowShootingPracticeGroundUi(t,e)mvars.qst_shootingPracticeStartUiPos=e or mvars.qst_shootingPracticeStartUiPos
mvars.qst_shootingPracticeOffsetType=t or mvars.qst_shootingPracticeOffsetType
...
function e.ShowShootingPracticeMarker(t)if t then
for n,e in pairs(I)do
if n~=t then
TppMarker.Disable(e)else
...
function e.HideAllShootingPracticeForUi()for t,e in pairs(I)do
...
function e.UpdateShootingPracticeUi()local t,e=TppGimmick.GetQuestShootingPracticeCount()TppUiCommand.SetDisplayTimerText("time_quest",t,e)end
local t={ruins_q60115=TppMotherBaseManagementConst.DESIGN_2027,sovietBase_q60110=TppMotherBaseManagementConst.DESIGN_2020,citadel_q60112=TppMotherBaseManagementConst.DESIGN_2021,outland_q60113=TppMotherBaseManagementConst.DESIGN_2022,pfCamp_q60114=TppMotherBaseManagementConst.DESIGN_2023,sovietBase_q60111=TppMotherBaseManagementConst.DESIGN_2024}local n={pfCamp_q39012={dataBaseId=TppMotherBaseManagementConst.ANIMAL_130},waterway_q39010={dataBaseId=TppMotherBaseManagementConst.ANIMAL_610},lab_q39011={dataBaseId=TppMotherBaseManagementConst.ANIMAL_2250}}function e.AcquireKeyItemOnMissionStart()for t,n in pairs(q)do
if e.IsCleard(t)then
TppTerminal.AcquireKeyItem{dataBaseId=n,isShowAnnounceLog=true}end
end
if TppStory.IsMissionCleard(11050)then
TppTerminal.AcquireKeyItem{dataBaseId=TppMotherBaseManagementConst.DESIGN_3014,pushReward=true}TppTerminal.AcquireKeyItem{dataBaseId=TppMotherBaseManagementConst.DESIGN_3015,pushReward=true}end
if TppDemo.IsPlayedMBEventDemo"DecisionHuey"then
TppTerminal.AcquireKeyItem{dataBaseId=TppMotherBaseManagementConst.DESIGN_3007,pushReward=true}end
if TppStory.IsMissionCleard(10280)then
TppTerminal.AcquireKeyItem{dataBaseId=TppMotherBaseManagementConst.DESIGN_3008,pushReward=true}TppTerminal.AcquireKeyItem{dataBaseId=TppMotherBaseManagementConst.DESIGN_3010,pushReward=true}end
if TppStory.IsMissionCleard(10100)then
TppTerminal.AcquireKeyItem{dataBaseId=TppMotherBaseManagementConst.DESIGN_3013,pushReward=true}end
if e.IsCleard"mtbs_q99060"then
TppTerminal.AcquireKeyItem{dataBaseId=TppMotherBaseManagementConst.PHOTO_1010,pushReward=true}end
...
TppTerminal.AcquireKeyItem{dataBaseId=428,isShowAnnounceLog=true}end
if e.IsCleard"ruins_q19010"then
vars.isRussianTranslatable=1
end
if e.IsCleard"outland_q19011"then
vars.isAfrikaansTranslatable=1
end
if e.IsCleard"hill_q19012"then
vars.isKikongoTranslatable=1
end
if e.IsCleard"commFacility_q19013"then
vars.isPashtoTranslatable=1
end
if e.IsCleard"tent_q99072"then
vars.mbmMasterGunsmithSkill=1
end
do
local t={{questName="mtbs_q99060",cassetteList={"tp_m_99060_01","tp_m_99060_02","tp_m_99060_03","tp_m_99060_04","tp_m_99060_05","tp_sp_01_03"}},{questName="sovietBase_q99030",cassetteList={"tp_m_10150_20","tp_m_10150_29","tp_m_10150_30"}},{questName="tent_q99040",cassetteList={"tp_m_10150_21","tp_m_10150_22","tp_m_10150_24","tp_m_10150_25"}},{questName="mtbs_q99011",cassetteList={"tp_c_00000_13","tp_m_10050_03"}},{storySequence=TppDefine.STORY_SEQUENCE.CLEARD_RESCUE_HUEY,cassetteList={"tp_m_10050_01"}},{storySequence=TppDefine.STORY_SEQUENCE.CLEARD_FLAG_MISSIONS_BEFORE_ENDRESS_PROXY_WAR,cassetteList={"tp_m_10160_06","tp_m_10160_07","tp_m_10160_08","tp_m_10160_09","tp_m_10160_10","tp_m_10160_05"}},{storySequence=TppDefine.STORY_SEQUENCE.CLEARD_MURDER_INFECTORS,cassetteList={"tp_m_10160_11","tp_m_10160_04","tp_c_00000_18","tp_m_10240_05","tp_m_10190_02","tp_m_10190_03"}},{demoName="DecisionHuey",cassetteList={"tp_m_10190_05","tp_m_10190_06","tp_m_10240_03","tp_m_10240_04","tp_m_10190_01"}}}for n,t in pairs(t)do
if((t.questName and e.IsCleard(t.questName))or(t.storySequence and(TppStory.GetCurrentStorySequence()>=t.storySequence)))or(t.demoName and TppDemo.IsPlayedMBEventDemo(t.demoName))then
TppCassette.Acquire{cassetteList=t.cassetteList,isShowAnnounceLog=true}end
end
end
do
local t=110
if e.IsCleard"cliffTown_q99080"and not TppMotherBaseManagement.IsExistStaff{uniqueTypeId=t}then
local e=TppMotherBaseManagement.GenerateStaffParameter{staffType="Unique",uniqueTypeId=t}TppMotherBaseManagement.DirectAddStaff{staffId=e}end
local t=111
if e.IsCleard"quest_q20095"and not TppMotherBaseManagement.IsExistStaff{uniqueTypeId=t}then
local e=TppMotherBaseManagement.GenerateStaffParameter{staffType="Unique",uniqueTypeId=t}TppMotherBaseManagement.DirectAddStaff{staffId=e}TppMotherBaseManagement.SetLockedTanFlag{locked=false}end
end
do
for t,n in pairs(p)do
if e.IsCleard(t)then
e.GetClearEmblem(t)end
end
end
end
if not TppGameSequence.IsMaster()then
function e.ResetAllQuestCleared()for t,e in pairs(TppDefine.QUEST_INDEX)do
gvars.qst_questClearedFlag[e]=false
end
end
function e.SetEntryQuest(t)e.ResetAllQuestCleared()t=t or{}for e,t in pairs(TppDefine.QUEST_INDEX)do
local e=TppDefine.QUEST_INDEX[e]gvars.qst_questOpenFlag[e]=false
gvars.qst_questActiveFlag[e]=false
gvars.qst_questClearedFlag[e]=false
end
for t,n in ipairs(t)do
local t
t=e.GetQuestIndex(n)gvars.qst_questClearedFlag[t]=true
end
e.UpdateActiveQuest{debugUpdate=true}end
function e.QuestForceActive(n)local e=TppDefine.QUEST_INDEX[questName]for e,t in pairs(TppDefine.QUEST_INDEX)do
local e=n==e
gvars.qst_questOpenFlag[t]=e
gvars.qst_questActiveFlag[t]=e
if e then
gvars.qst_questClearedFlag[t]=false
end
end
end
function e.QuestForceActiveTable(n)local function e(n,e,t)for a,e in ipairs(e)do
if n==e and t[e]==false then
t[e]=true
return true
end
end
...
for e,t in pairs(TppDefine.QUEST_INDEX)do
local e=TppDefine.QUEST_INDEX[e]gvars.qst_questOpenFlag[e]=false
gvars.qst_questActiveFlag[e]=false
gvars.qst_questClearedFlag[e]=true
end
local t={}for n,e in ipairs(n)do
t[e]=false
end
for s,e in pairs(TppDefine.QUEST_INDEX)do
local a=false
for n,e in ipairs(n)do
if s==e and t[e]==false then
t[e]=true
a=true
end
end
...
function e.ForceClearWithSave(n,t)for a,t in ipairs(t)do
e.ClearWithSave(n,t)end
end
end
...
do
mvars.qaDebug.forceClear=false
e.AddDebugMenu("LuaQuest","forceClearCurrentQuest","bool",mvars.qaDebug,"forceClear")mvars.qaDebug.forceClearAllActiveQuest=false
e.AddDebugMenu("LuaQuest","forceClearAllActiveQuest","bool",mvars.qaDebug,"forceClearAllActiveQuest")mvars.qaDebug.clearFlagAll=false
e.AddDebugMenu("LuaQuest","clearFlagAll","bool",mvars.qaDebug,"clearFlagAll")end
end
function e.QAReleaseDebugUpdate()if mvars.qaDebug.clearFlagAll then
e.QAReleaseDebugClearAll()mvars.qaDebug.clearFlagAll=false
end
if mvars.qaDebug.forceClear then
e.ClearWithSave(TppDefine.QUEST_CLEAR_TYPE.CLEAR)mvars.qaDebug.forceClear=false
end
if mvars.qaDebug.forceClearAllActiveQuest then
e.QAReleaseClearAllActiveQuest()mvars.qaDebug.forceClearAllActiveQuest=false
end
end
function e.QAReleaseDebugClearAll()for n,t in pairs(TppDefine.QUEST_INDEX)do
gvars.qst_questOpenFlag[t]=false
gvars.qst_questClearedFlag[t]=false
gvars.qst_questRepopFlag[t]=false
gvars.qst_questActiveFlag[t]=false
end
e.UpdateActiveQuest(true)e.Save()end
function e.QAReleaseFullOpen()for n,t in pairs(TppDefine.QUEST_INDEX)do
gvars.qst_questOpenFlag[t]=true
gvars.qst_questClearedFlag[t]=false
gvars.qst_questRepopFlag[t]=false
gvars.qst_questActiveFlag[t]=false
end
e.UpdateActiveQuest(true)e.Save()end
function e.QAReleaseClearAllActiveQuest()for n,t in pairs(TppDefine.QUEST_INDEX)do
gvars.qst_questClearedFlag[t]=gvars.qst_questActiveFlag[t]or gvars.qst_questClearedFlag[t]end
e.UpdateActiveQuest(true)e.Save()end
end
return e