...
mvars.res_rankLimitedSetting={}if a.sequence.rankLimitedSetting then
...
e.SetMissionScoreTable(a.score.missionScoreTable)else
e.SetMissionScoreTable{baseTime={S=300,A=600,B=1800,C=5580,D=6480,E=8280}}end
end
function e.OnReload(a)e.Init(a)end
function e.OnMessage(e,e,e,e,e,e,e)end
function e.OnMissionCanStart()end
function e.SetRankTable(e)if not s(e)then
return
end
mvars.res_rankTable=e
end
function e.SaveBestCount(a)local e=svars
e.bestScoreTime=0
...
for e=1,r do
s=TppDefine.MISSION_CLEAR_RANK_LIST[e]local s=mvars.res_missionScoreTable.baseTime[s]*i
...
t=e.RANK_BASE_SCORE[s]return t,a
end
local i=1/1e3
local a=60
local t=(a*60)*5
...
local s=(a*60)*4
local s=(a*60)*.5
function e.CalcTimeScore(n,r)if not mvars.res_missionScoreTable then
return
end
local s=svars
local e=TppDefine.MISSION_CLEAR_RANK_LIST[r]local e=mvars.res_missionScoreTable.baseTime[e]local e=e-(s.bestScoreTime*i)if e<0 then
e=0
end
local e=e*a
...
local a=999999
local r=-999999
function e.CalcTotalScore(t)local e=0
local s=0
...
a=SsdMissionList.MISSION_ENUM[tostring(vars.missionCode)]elseif t==TppDefine.MISSION_TYPE.FLAG then
a=SsdMissionList.MISSION_ENUM[tostring(gvars.fms_currentFlagMissionCode)]end
if a then
...
function e.DecideMissionClearRank()local e
local s=svars.bestScore
local a=#TppDefine.MISSION_CLEAR_RANK_LIST
if not mvars.res_noMissionClearRank then
for a=1,a do
local t=TppDefine.MISSION_CLEAR_RANK_LIST[a]if s>=mvars.res_rankTable[t]then
e=a
break
end
end
...
function e.SetBestRank(s,e)if(e<TppDefine.MISSION_CLEAR_RANK.NOT_DEFINED)or(e>#TppDefine.MISSION_CLEAR_RANK_LIST)then
return
end
local a
...
local a=SsdMissionList.MISSION_ENUM[tostring(a)]if not a then
return
end
...
function e.DebugUpdate()end
function e.Messages()end
return e