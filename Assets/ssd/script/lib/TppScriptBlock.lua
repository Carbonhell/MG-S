...
local n=-127
function e.DeclareSVars()return{{name="sbl_scriptBlockName",type=TppScriptVars.TYPE_UINT32,arraySize=t,value=0,save=false,sync=false,wait=false,category=TppScriptVars.CATEGORY_MISSION},{name="sbl_scriptBlockPack",type=TppScriptVars.TYPE_UINT32,arraySize=t,value=0,save=false,sync=false,wait=false,category=TppScriptVars.CATEGORY_MISSION},{name="sbl_isActive",type=TppScriptVars.TYPE_BOOL,arraySize=t,value=false,save=false,sync=false,wait=false,category=TppScriptVars.CATEGORY_MISSION},nil}end
function e.GetCurrentPackListName(e)local t=ScriptBlock.GetScriptBlockId(e)if t~=ScriptBlock.SCRIPT_BLOCK_ID_INVALID then
local e=mvars.sbl_scriptBlockPackList[e]if not e then
return
end
local t=svars.sbl_scriptBlockPack[t]if t==0 then
return
end
...
function e.IsRequestActivate(e)local e=e or r()if mvars.sbl_scriptBlockState[e]==TppDefine.SCRIPT_BLOCK_STATE.ACTIVATE_REQUESTED then
return true
else
return false
end
end
function e.SetScriptBlockState(c,e)local c=c or r()if c==ScriptBlock.SCRIPT_BLOCK_ID_INVALID then
return false
end
if c>=t then
return false
...
function e.Load(c,n,i,t)if gvars.exc_processingForDisconnect then
return
end
local r=true
if t==false then
...
e.SaveScriptBlockId(c,t)local c=e.FindPackList(c,n)if c then
svars.sbl_scriptBlockPack[t]=l(n)if r then
...
function e.UnloadAll()for e=0,t-1 do
svars.sbl_scriptBlockPack[e]=0
ScriptBlock.Load(e,"")end
end
...
function e.PreloadRequestOnMissionStart(e)if not c(e)then
return
end
local c=mvars.sbl_preloadRequestTable
for t,e in pairs(e)do
local l,t=next(e)local e={}e[l]=t
table.insert(c,e)end
end
function e.DEBUG_PreloadRequest(t)if not c(t)then
return
end
...
function e.PreloadSettingOnMissionStart()local t=mvars.sbl_preloadRequestTable
if(Tpp.IsQARelease()or nil)and c(e.DEBUG_preloadRequestTable)then
for c,e in pairs(e.DEBUG_preloadRequestTable)do
local c
local l,r=next(e)for e,t in pairs(t)do
local t,r=next(t)if t==l then
c=e
end
end
local e={}e[l]=r
...
function e.ReloadScriptBlock()if gvars.exc_processingForDisconnect then
return
end
mvars.sbl_currentScriptBlockPackInfo={}for t=0,(t-1)do
...
local e=ScriptBlock.GetScriptBlockId(e)if e~=ScriptBlock.SCRIPT_BLOCK_ID_INVALID then
mvars.sbl_scriptBlockStrCode32PackList[e]={}for c,t in pairs(t)do
...
function e.ResolveSavedScriptBlockInfo(t)local l=svars.sbl_scriptBlockName[t]local n=svars.sbl_scriptBlockPack[t]if l==0 then
return
end
if n==0 then
return
...
local r=ScriptBlock.GetScriptBlockId(c)if r~=ScriptBlock.SCRIPT_BLOCK_ID_INVALID then
mvars.sbl_currentScriptBlockPackInfo[r]={scriptBlockNameHash=l,packListKeyHash=n}else
