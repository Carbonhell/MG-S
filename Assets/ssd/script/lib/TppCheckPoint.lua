...
local o=0
function e.DeclareSVars()return{{name="chk_checkPointName",arraySize=TppDefine.CHECK_POINT_MAX,type=TppScriptVars.TYPE_UINT32,value=0,save=false,sync=false,wait=false,category=TppScriptVars.CATEGORY_MISSION},{name="chk_checkPointEnable",arraySize=TppDefine.CHECK_POINT_MAX,type=TppScriptVars.TYPE_BOOL,value=0,save=false,sync=false,wait=false,category=TppScriptVars.CATEGORY_MISSION},nil}end
...
local t="trap_"..i
local e={msg="Enter",sender=t,func=function(n,n)e.Update{checkPoint=i,trapName=t,safetyCurrentPosition=true}end,option={isExecFastTravel=true}}table.insert(n,e)end
...
function e.OnMessage(s,a,o,t,n,i,c)Tpp.DoMessage(e.messageExecTable,TppMission.CheckMessageOption,s,a,o,t,n,i,c)end
function e.Enable(n)e._SetEnable(n,true)end
function e.Disable(n)e._SetEnable(n,false)end
function e.Reset()gvars.mis_checkPoint=o
end
...
function e.Update(n)if vars.missionCode==TppDefine.SYS_MISSION_ID.SELECT then
return
end
local o
local l
...
local n
if s then
e.Reset()TppPlayer.SetInitialPositionToCurrentPosition()elseif r then
if Gimmick.IsVehicleFultonUpdating()then
return
end
if Player.CanSaveAsCheckPoint and Player.CanSaveAsCheckPoint()then
e.Reset()TppPlayer.SetInitialPositionToCurrentPosition()Player.NotifyCheckPointTrapName(c)local e=TppSave.GetGameSaveFileName()if TppSave.IsSavingWithFileName(e)or TppSave.HasQueue(e)then
n=true
end
end
else
...
function e.DebugUpdate()local i=DebugText.NewContext()if mvars.debug.showCheckPointList then
DebugText.Print(i,{.5,.5,1},"TppCheckPoint: showCheckPointList")for o,n in pairs(mvars.mis_checkPointList)do
if t(n)and e.IsEnable(n)then
DebugText.Print(i,{1,1,1},n)end
end
end
end
