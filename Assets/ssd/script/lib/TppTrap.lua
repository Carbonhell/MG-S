...
local t=DebugText.NewContext()if a.debug.showTrapStatus and a.trp_variableTrapList then
r(t,{.5,.5,1},"LuaSystem TRP.trapStatus")local e=1
...
if a>=e then
local e=n.name
local a=svars.trp_variableTrapEnable[a]r(t,{.5,.5,1},"trapName = "..(tostring(e)..(", status = "..tostring(a))))end
end
end
end
...
function a.DeclareSVars()return{{name="trp_variableTrapEnable",arraySize=n,type=TppScriptVars.TYPE_BOOL,value=true,save=false,sync=false,wait=false,category=TppScriptVars.CATEGORY_MISSION},nil}end
function a.Enable(e)a.ChangeTrapState(e,true)end
function a.Disable(e)a.ChangeTrapState(e,false)end
function a.ChangeTrapState(e,r)local t=mvars.trp_variableTrapTable[e]if t==nil then
return
end
local i=t.index
local n
if t.type==TppDefine.TRAP_TYPE.NORMAL then
n=a.ChangeNormalTrapState(e,r)elseif t.type==TppDefine.TRAP_TYPE.TRIGGER then
n=a.ChangeTriggerTrapState(e,r)else
n=a.ChangeNormalTrapState(e,r)end
if n then
svars.trp_variableTrapEnable[i]=r
end
end
function a.ChangeNormalTrapState(a,e)return TppDataUtility.SetEnableDataFromIdentifier("VariableTrapIdentifier",a,e)end
