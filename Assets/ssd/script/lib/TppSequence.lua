local e={}local a={}local r={}local _=256
local c=0
local S=180
local n=60
local d=Fox.StrCode32
local T=Tpp.IsTypeFunc
local i=Tpp.IsTypeTable
local t=GkEventTimerManager.Start
local n=TppScriptVars.SVarsIsSynchronized
e.MISSION_PREPARE_STATE=Tpp.Enum{"START","WAIT_INITALIZE","WAIT_TEXTURE_LOADING","END_TEXTURE_LOADING","WAIT_ALL_MEMBER_IF_COOP","WAIT_BASE_DEFENSE_RESULT","WAIT_SAVING_FILE","END_SAVING_FILE","WAIT_INITIAL_LOAD","END_CHECK_EXCEPTION","FINISH"}local function s(n)local e=mvars.seq_sequenceTable
if e then
return e[n]end
end
local function p(n)local e=mvars.seq_sequenceNames
if e then
return s(e[n])end
end
...
function e.SetNextSequence(r,n)local s=nil
if mvars.seq_sequenceNames then
s=mvars.seq_sequenceNames[r]end
if s==nil then
return
end
local t=false
...
local n=TppMission.GetMissionName()local e=e.GetCurrentSequenceName()Mission.SetMiniText(0,r)end
svars.seq_sequence=s
return
end
if Tpp.IsQARelease()or nil then
local n=TppMission.GetMissionName()local e=e.GetCurrentSequenceName()end
end
...
n[e]={name=i,type=TppScriptVars.TYPE_UINT32,value=d(s),save=true,sync=true,category=TppScriptVars.CATEGORY_MISSION}elseif t=="table"then
n[e]=s
end
e=e+1
end
...
local s=1
r={"Seq_Mission_Prepare","Seq_Accepted_Invitation"}e.SYS_SEQUENCE_LENGTH=#r
...
mvars.seq_canMissionStartWaitStartTime=Time.GetRawElapsedTimeSinceStartUp()mvars.isTextureLoadFinished=false
TppMain.OnEnterMissionPrepare()TppMain.DisablePause()gvars.canExceptionHandling=true
...
TppUiStatusManager.ClearStatus"AnnounceLog"TppUiStatusManager.SetStatus("AnnounceLog","SUSPEND_LOG")end
end
end,CanMissionGameStart=function()if SsdFlagMission.IsLoaded()then
return true
end
return false
end,MissionGameStart=function()if InvitationManagerController.IsGoingCoopMission()then
TppMain.DisableBlackLoading()e.SetNextSequence"Seq_Accepted_Invitation"elseif not a.Seq_Mission_Prepare.CanMissionGameStart()then
t("Timer_WaitStartingGame",s)elseif mvars.seq_demoSequneceList[mvars.seq_missionStartSequence]then
...
e.SetMissionGameStartSequence()if gvars.exc_processingExecptionType~=0 then
return
end
TppUI.FadeIn(TppUI.FADE_SPEED.FADE_NORMALSPEED,"FadeInOnGameStart",TppUI.FADE_PRIORITY.MISSION,{exceptGameStatus=n})end,SkipTextureLoadingWait=function()if mvars.seq_skipTextureLoadingWait then
return true
end
end,DEBUG_TextPrint=function(e)if not DebugText then
return
end
local n=DebugText.NewContext()DebugText.Print(n,{.5,.5,1},e)end,OnUpdate=function(n)if(mvars.seq_missionPrepareState<e.MISSION_PREPARE_STATE.END_TEXTURE_LOADING)then
...
local a=false
local o=Mission.GetTextureLoadedRate()local a=false
if vars.missionCode~=TppDefine.SYS_MISSION_ID.INIT then
local e=TppMission.CanStart()if Player.IsInstanceValid()or vars.missionCode>=6e4 then
a=e
end
else
a=TppMission.CanStart()end
if n.SkipTextureLoadingWait()then
o=1
end
local c=0
local r=_
local d=Time.GetRawElapsedTimeSinceStartUp()local l=d-mvars.seq_canMissionStartWaitStartTime
if DebugText then
...
TppMain.OnTextureLoadingWaitStart()mvars.seq_textureLoadWaitStartTime=d
end
...
i=DebugPad.IsScannedAorB()if i then
...
if DebugText then
n.DEBUG_TextPrint(string.format("033.0 > [MissionPrepare] Waiting Mission.CanStart() is true. canMissionStartWaitingTime =  %02.2f[s] : TIMEOUT = %02.2f[s]",l,S))end
end
if not i then
if DebugText then
n.DEBUG_TextPrint(string.format("034.0 > [MissionPrepare] Waiting texture loading. TimeOutRemain = %02.2f[s], textureLoadRatio = %03.2f /.%03.2f",r,o*100,u*100))if mvars.seq_forceStopWhileNotPressedPad then
...
if(mvars.seq_missionPrepareState<e.MISSION_PREPARE_STATE.WAIT_BASE_DEFENSE_RESULT)then
...
mvars.seq_missionPrepareState=e.MISSION_PREPARE_STATE.WAIT_BASE_DEFENSE_RESULT
end
if(mvars.seq_missionPrepareState<e.MISSION_PREPARE_STATE.WAIT_SAVING_FILE)then
...
TppMain.OnMissionCanStart()return
end
if(mvars.seq_missionPrepareState<e.MISSION_PREPARE_STATE.WAIT_INITIAL_LOAD)then
if not n.CanMissionGameStart()then
n.DEBUG_TextPrint"065.0 > Wait Initial Mission Loading."return
end
mvars.seq_missionPrepareState=e.MISSION_PREPARE_STATE.WAIT_INITIAL_LOAD
if TppUiCommand.IsEndLoadingTips()then
TppUI.FinishLoadingTips()t("Timer_WaitStartingGame",s)else
if gvars.waitLoadingTipsEnd then
mvars.seq_nowWaitingPushEndLoadingTips=true
TppUiCommand.PermitEndLoadingTips()else
TppUI.FinishLoadingTips()t("Timer_WaitStartingGame",s)end
end
end
if mvars.seq_missionPrepareState<e.MISSION_PREPARE_STATE.END_CHECK_EXCEPTION then
mvars.seq_missionPrepareState=e.MISSION_PREPARE_STATE.END_CHECK_EXCEPTION
if gvars.exc_processingExecptionType~=0 and gvars.waitLoadingTipsEnd then
TppUI.FinishLoadingTips()t("Timer_WaitStartingGame",s)end
end
end}a.Seq_Accepted_Invitation={OnEnter=function()TppMission.GoToCoopLobby()end}function e.IsMissionPrepareFinished()if mvars.seq_missionPrepareState then
...
function e.SaveMissionStartSequence()local n=e.SYS_SEQUENCE_LENGTH+1
...
if d(s)==svars.dbg_seq_sequenceName then
e=n
break
end
end
...
function e.DoOnEndMissionPrepareFunction()NamePlateMenu.SetDefaultNamePlateIfSetBegginerNamePlate()if mvars.seq_onEndMissionPrepareFunction then
mvars.seq_onEndMissionPrepareFunction()end
end
function e.IsFirstLandStart()if(not mvars.seq_demoSequneceList[mvars.seq_missionStartSequence])and(mvars.seq_missionStartSequence==(e.SYS_SEQUENCE_LENGTH+1))then
return true
else
return false
end
end
function e.IsLandContinue()if(not mvars.seq_demoSequneceList[mvars.seq_missionStartSequence])and(e.GetContinueCount()>0)then
return true
else
return false
end
...
local s=p(svars.seq_sequence)if s==nil then
return
end
...
function e.OnMessage(t,s,n,o,a,r,i)if mvars.seq_sequenceTable==nil then
return
end
local e=mvars.seq_sequenceTable[mvars.seq_currentSequence]if e==nil then
return
...
Tpp.DoMessage(e,TppMission.CheckMessageOption,t,s,n,o,a,r,i)end
...
DebugText.Print(n,{.5,.5,1},"LuaSystem SEQ.showCurrSequence")DebugText.Print(n," current_sequence = "..tostring(u(s.seq_sequence)))end
if e.debug.showSequenceHistory then
DebugText.Print(n,{.5,.5,1},"LuaSystem SEQ.showSeqHistory")for e,s in ipairs(e.debug.seq_sequenceHistory)do
DebugText.Print(n," seq["..(tostring(e)..("] = "..tostring(s))))end
end
end
end
return e