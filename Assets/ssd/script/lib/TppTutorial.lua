...
function e.DispGuide(n,T,p)local t=TppStory.GetCurrentStorySequence()local i=not Tpp.IsNotAlert()if T==e.DISPLAY_OPTION.TIPS then
e.DispTipsGuide(n,t,i,p)elseif T==e.DISPLAY_OPTION.CONTROL then
e.DispControlGuide(n,t,i,p)elseif T==e.DISPLAY_OPTION.TIPS_CONTROL then
e.DispControlGuide(n,t,i)e.DispTipsGuide(n,t,i)elseif T==e.DISPLAY_OPTION.PAUSE_CONTROL then
e.DispControlGuide(n,t,i,nil,true)elseif T==e.DISPLAY_OPTION.TIPS_IGONORE_RADIO then
e.DispTipsGuide(n,t,i,p,false,true)elseif T==e.DISPLAY_OPTION.CONTROL_IGONORE_RADIO then
e.DispControlGuide(n,t,i,p,false,true)elseif T==e.DISPLAY_OPTION.TIPS_IGONORE_DISPLAY then
e.DispTipsGuide(n,t,i,p,false,false,true)end
end
function e.DispTipsGuide(n,t,p,T,i,i,i)local n=TppDefine.TIPS[n]if not n then
return
end
local i=e.TipsStoryFlag[n]if i then
if t<i then
return
end
end
local i=e.TipsLocation[n]if i then
if not i()then
return
end
end
...
local i=e.IgnoredTipsGuideInMission[n]if i then
...
local t=true
local i=false
local e=e.TipsExceptTime[n]if e then
t=e.isOnce
i=e.isOnceThisGame
end
local e=T
end
function e.DispControlGuide(T,t,o,_,r,p)local n=TppDefine.CONTROL_GUIDE[T]if not n then
return
end
local i=e.ControlStoryFlag[n]if i then
if t<i then
return
end
end
...
local i=e.IgnoredControlGuideInMission[n]if i then
for e,n in pairs(i)do
if vars.missionCode==e then
return
end
...
local t=true
local i=false
local e=e.ControlExceptTime[n]if e then
t=e.isOnce
i=e.isOnceThisGame
end
local e=_
TppUI.ShowControlGuide{actionName=T,isOnce=t,isOnceThisGame=i,time=e,pauseControl=r,ignoreRadio=p}end
function e.OnIconFultonShown(t,n,i)local n=GameObject.GetTypeIndex(n)if e.FultonTipsGuideMatchTable[n]then
if n~=TppGameObject.GAME_OBJECT_TYPE_FULTONABLE_CONTAINER then
e.DispGuide(e.FultonTipsGuideMatchTable[n],e.DISPLAY_OPTION.TIPS)else
if i==1 then
e.DispGuide(e.FultonTipsGuideMatchTable[n],e.DISPLAY_OPTION.TIPS)end
end
end
end
function e.OnVehicleDrive(i,n)local n=GameObject.SendCommand(n,{id="GetVehicleType"})if e.AttackVehicleTable[n]then
...
function e.OnPickUpCollection(i,i,n)if n==TppCollection.TYPE_DIAMOND_SMALL or n==TppCollection.TYPE_DIAMOND_LARGE then
e.DispGuide("DIAMOND",e.DISPLAY_OPTION.TIPS)elseif TppCollection.IsHerbByType(n)then
e.DispGuide("PLANT",e.DISPLAY_OPTION.TIPS)if not TppUiCommand.IsDispGuide"TipsGuide"then
local n=e.PlantRadioMatchTable[n]if n then
e.PlayTutorialRadioOnly(n,{delayTime="mid"})end
end
elseif n==TppCollection.TYPE_DEVELOPMENT_FILE then
e.DispGuide("DEV_DOCUMENT",e.DISPLAY_OPTION.TIPS)elseif n==TppCollection.TYPE_EMBLEM then
e.DispGuide("EMBLEM",e.DISPLAY_OPTION.TIPS)elseif n==TppCollection.TYPE_SHIPPING_LABEL then
e.DispGuide("BOX_MOVE",e.DISPLAY_OPTION.TIPS)elseif n>=TppCollection.TYPE_MATERIAL_CM_0 and n<=TppCollection.TYPE_MATERIAL_BR_7 then
e.DispGuide("MATERIAL",e.DISPLAY_OPTION.TIPS)end
end
local t=function(n)local e={TppEquip.EQP_WP_10101,TppEquip.EQP_WP_10102,TppEquip.EQP_WP_10103,TppEquip.EQP_WP_10104,TppEquip.EQP_WP_10105,TppEquip.EQP_WP_10107,TppEquip.EQP_WP_10116,TppEquip.EQP_WP_10125,TppEquip.EQP_WP_10134,TppEquip.EQP_WP_10136,TppEquip.EQP_WP_10214,TppEquip.EQP_WP_10216,TppEquip.EQP_WP_60013,TppEquip.EQP_WP_60015,TppEquip.EQP_WP_60016,TppEquip.EQP_WP_60114,TppEquip.EQP_WP_60115,TppEquip.EQP_WP_60116,TppEquip.EQP_WP_60117,TppEquip.EQP_WP_60325,TppEquip.EQP_WP_60326,TppEquip.EQP_WP_60327}for i,e in pairs(e)do
if e==n then
return true
end
end
end
function e.OnPlayerHoldWeapon(n,i,p,T)if T==1 then
...
function e.OnEquipHudClosed(t,i,n)if n==TppEquip.EQP_TYPE_Throwing then
e.DispGuide("THROW_EQUIP",e.DISPLAY_OPTION.TIPS)elseif n==TppEquip.EQP_TYPE_Placed then
local n=TppEquip.GetSupportWeaponTypeId(i)if n==TppEquip.SWP_TYPE_CaptureCage then
...
local T
local r
n,_,p,t,T,r=TppEquip.GetAmmoInfo(i)if n~=0 and t~=0 then
e.DispGuide("UNDER_BARREL",e.DISPLAY_OPTION.TIPS_CONTROL)end
end
end
function e.OnWeaponPutPlaced(i,n)local n=TppEquip.GetSupportWeaponTypeId(n)if n==TppEquip.SWP_TYPE_C4 then
e.DispGuide("C4_EXPLODING",e.DISPLAY_OPTION.CONTROL)end
end
function e.OnAmmoStackEmpty(e,e,e)end
function e.OnSuppressorIsBroken()TppRadio.PlayCommonRadio(TppDefine.COMMON_RADIO.SUPPRESSOR_BROKEN,true)end
function e.StartInvestigate(i,n,i)if n==0 then
return
end
e.PlayTutorialRadioOnly("f1000_rtrg4450",{delayTime="long"})end
function e.EndInvestigate(i,n,i)if n==0 then
return
end
e.PlayTutorialRadioOnly("f1000_rtrg4460",{delayTime="long"})end
local i=function(n)local e=PlayerConstants.ITEM_COUNT-1
for e=0,e do
...
function e.DispGuide_PhatomCigar(n)if not i(TppEquip.EQP_IT_TimeCigarette)then
return
end
if vars.playerPhase<TppGameObject.PHASE_ALERT and vars.playerPhase>TppGameObject.PHASE_SNEAK then
e.DispGuide(n,e.DISPLAY_OPTION.TIPS)end
end
function e.DispGuide_Weather(n)if e.WeatherTipsGuideMatchTable[n]then
e.DispGuide(e.WeatherTipsGuideMatchTable[n],e.DISPLAY_OPTION.TIPS)end
end
...
function e.OnDiscoveredBySniper()TppRadio.PlayCommonRadio(TppDefine.COMMON_RADIO.DISCOVERED_BY_SNIPER,true)end
function e.OnIconSwitchShown(T,t,n,i)local n=TppGimmick.GetGimmickID(t,n,i)if not n then
return
end
local n=mvars.gim_connectPowerCutAreaTable[n]if n then
e.DispGuide("ELECTRICITY",e.DISPLAY_OPTION.TIPS)end
end
function e.OnIconClimbOnShown(i,n)if n==1 then
e.DispGuide("CRACK_CLIMB",e.DISPLAY_OPTION.TIPS)end
end
function e.OnBulletGuardArmor(i,T,i,n)if BulletGuardArmorMessageFlag==nil then
return
end
local t=BulletGuardArmorMessageFlag.BROKEN_HELMET
local i=BulletGuardArmorMessageFlag.IS_HIT_ARMOR
if bit.band(n,t)==t then
e.DispGuide("BULLET_PENETRATE",e.DISPLAY_OPTION.TIPS)elseif bit.band(n,i)==i then
if not TppDamage.IsActiveByAttackId(T)then
e.DispGuide("BULLET_PENETRATE_FAIL",e.DISPLAY_OPTION.TIPS)end
end
end
function e.OnMarking(T,T,i,t)if t~=n"Player"then
return
end
...
function e.OnFultonRecovered(n)local i=GameObject.GetTypeIndex(n)if i==TppGameObject.GAME_OBJECT_TYPE_VEHICLE then
if n~=GameObject.CreateGameObjectId("TppVehicle2",0)then
e.PlayTutorialRadioOnly("f1000_rtrg4540",{delayTime="long"})end
end
end
function e.OnRadioStart(n)local n=e.RadioTipsMatchTable[n]if n then
e.DispGuide(n,e.DISPLAY_OPTION.TIPS_IGONORE_RADIO)end
end
function e._UnregisterIntelRadioAfterPlayed(t)if TppMission.IsFreeMission(vars.missionCode)then
return
end
...
function e.OpenTipsOnCurrentStory()end
function e.SetIgnoredControlGuideInMission(i,n,t)local n=TppDefine.CONTROL_GUIDE[n]if not n then
return
end
if t then
if not e.IgnoredControlGuideInMission[n]then
e.IgnoredControlGuideInMission[n]={}end
...
function e.SetIgnoredGuideInMission(i,n,t)if not _(i)or not o(n)then
return
end
e.SetIgnoredTipsGuideInMission(i,n,t)e.SetIgnoredControlGuideInMission(i,n,t)end
function e.SetNoGuideMission(n,i)if not _(n)then
return
end
if i then
...
function e.SetIgnoredTipsGuideInMission(i,n,t)local n=TppDefine.TIPS[n]if not n then
return
end
if t then
if not e.IgnoredTipsGuideInMission[n]then
e.IgnoredTipsGuideInMission[n]={}end
...
function e.PlayTutorialRadioOnly(i,n)if not TppUI.IsEnableToShowGuide()then
return
end
if e.NoGuideMission[vars.missionCode]then
return
...
e.PlayRadio(i,n)end
function e.PlayRadio(e,i)local n={delayTime="short"}if i then
n=i
end
if r(e)then
local i={}for n,e in pairs(e)do
if not TppRadio.IsPlayed(e)then
table.insert(i,e)end
end
TppRadio.Play(i,n)else
if not TppRadio.IsPlayed(e)then
TppRadio.Play(e,n)end
end
end
function e.SetIntelRadio()if e.NoIntelRadioMission[vars.missionCode]then
return
end
...
table.remove(e,i)end
end
end
TppRadio.ChangeIntelRadio(e)end
function e.StartHelpTipsMenuOnlyAnnounce(e)if not e then
return
end
...
TppRadio.Play(e.startRadio)end
HelpTipsMenuSystem.SetPageOpenedWithAnnounce(e.tipsTypes)end
function e.StartHelpTipsMenu(n)if not SsdHelpTipsManager.IsIdle()then
return
end
if not n then
return
...
e._RequestOpenHelpTips(n.startRadio,n.tipsRadio,n.tipsTypes,n.endFunction)end
function e.IsHelpTipsMenu()if SsdHelpTipsManager.IsIdle()then
return false
end
return true
end
function e.ResetHelpTipsSettings()end
function e._RequestOpenHelpTips(i,t,T,n)local e={}e.startRadio={}e.tipsRadio={}e.tipsTypes={}e.onEnd=nil
if i then
local n,i=TppRadio.GetRadioNameAndRadioIDs(i)e.startRadio.radioName=n
e.startRadio.groupName=i
e.startRadio.delayTime=0
end
if t then
local n,i=TppRadio.GetRadioNameAndRadioIDs(t)e.tipsRadio.radioName=n
...
Tpp.DEBUG_DumpTable(e)SsdHelpTipsManager.RequestOpen(e)end
function e.OnReload(n)e.OpenTipsOnCurrentStory()end
function e.Init(n)e.OpenTipsOnCurrentStory()end
return e