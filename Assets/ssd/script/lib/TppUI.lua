...
local d=0
e.FADE_SPEED={FADE_MOMENT=0,FADE_HIGHESTSPEED=.5,FADE_HIGHSPEED=1,FADE_NORMALSPEED=2,FADE_LOWSPEED=4,FADE_LOWESTSPEED=8}if SsdFadeManager then
e.FADE_PRIORITY={SYSTEM=SsdFadeManager.PRIORITY_SYSTEM,DEMO=SsdFadeManager.PRIORITY_DEMO,RESULT=SsdFadeManager.PRIORITY_RESULT,MISSION=SsdFadeManager.PRIORITY_MISSION,FLAG=SsdFadeManager.PRIORITY_FLAG,QUEST=SsdFadeManager.PRIORITY_QUEST,USER=SsdFadeManager.PRIORITY_USER}else
e.FADE_PRIORITY={}end
e.ANNOUNCE_LOG_TYPE={updateMissionInfo="announce_mission_info_update",updateMap="announce_map_update",recoverTarget="announce_target_extract",eliminateTarget="announce_target_eliminate",destroyTarget="announce_target_destroy",achieveObjectiveCount="announce_objective_complete_num",achieveAllObjectives="announce_objective_complete",getIntel="announce_get_intel_file",target_died="announce_target_died",target_eliminate_failed="announce_target_eliminate_failed",target_extract_failed="announce_target_extract_failed",staff_dead="announce_staff_dead",staff_dying="announce_staff_dying",closeOutOfMissionArea="announce_mission_area_warning",getDiamond="announce_get_diamond",missionListUpdate="announce_mission_list_update",sunset="announce_sunset",sunrise="announce_sunrise",weather_sunny="announce_weather_sunny",weather_cloudy="announce_weather_cloudy",weather_rainy="announce_weather_rain",weather_sandstorm="announce_weather_sandstorm",weather_foggy="announce_weather_fog",getPoster="announce_get_gravure",task_complete="announce_task_complete",quest_add="announce_quest_add",quest_complete="announce_quest_complete",quest_delete="announce_quest_delete",quest_list_update="announce_quest_list_update",mine_quest_log="announce_quest_disposal_mine",quest_get_photo="announce_get_photo",find_keyitem="announce_find_keyitem",get_tape="announce_get_tape",add_alt_machine="announce_add_alt_machine",get_blueprint="announce_get_blueprint",recoveredFilmCase="announce_get_film_case",find_processed_res="announce_find_processed_res",find_diamond="announce_find_diamond",find_plant="announce_find_plant"}e.ANNOUNCE_LOG_PRIORITY={"eliminateTarget","recoveredFilmCase","recoverTarget","destroyTarget","achieveAllObjectives","achieveObjectiveCount","getIntel","updateMissionInfo","updateMap"}e.EMBLEM_ANNOUNCE_LOG_TYPE={[Fox.StrCode32"front"]="find_em_front",[Fox.StrCode32"base"]="find_em_back",[Fox.StrCode32"word"]="find_em_string"}function e.Messages()return Tpp.StrCode32Table{UI={{msg="EndFadeIn",func=e.EnableGameStatusOnFade,option={isExecMissionClear=true,isExecDemoPlaying=true,isExecGameOver=true,isExecFastTravel=true}},{msg="EndFadeOut",func=e.DisableGameStatusOnFadeOutEnd,option={isExecMissionClear=true,isExecDemoPlaying=true,isExecGameOver=true,isExecFastTravel=true}},{msg="ConfigurationUpdated",func=function()if vars.missionCode==TppDefine.SYS_MISSION_ID.INIT then
return
end
...
TppSave.SaveConfigData()end,option={isExecGameOver=true}},{msg="StartMissionTelopFadeOut",func=function()TppSoundDaemon.ResetMute"Telop"end}},Network={{msg="AddSessionMember",func=function()e._UpdateCoopMissionPauseMenuList()end},{msg="DeleteSessionMember",func=function()e._UpdateCoopMissionPauseMenuList()end}}}end
local function s(e)if type(e)=="string"then
return Fox.StrCode32(e)elseif type(e)=="number"then
return e
end
return nil
end
function e.FadeIn(a,o,n,i)local o=s(o)if i then
...
if not n then
n=e.FADE_PRIORITY.USER
end
end
t(a,o,n)e.EnableGameStatusOnFadeInStart()end
...
function e.FadeOut(d,u,t,i)local o,a
if n(i)then
o=i.setMute
a=i.exceptGameStatus
end
local n=s(u)e.DisableGameStatusOnFade(a)if o then
...
if SsdFadeManager then
if not t then
t=e.FADE_PRIORITY.USER
end
end
_(d,n,t)end
function e.ShowAnnounceLog(o,a,t,n,i)if gvars.ini_isTitleMode then
return
end
local e=e.ANNOUNCE_LOG_TYPE[o]if e then
if n then
TppUiCommand.AnnounceLogDelayTime(n)end
TppUiCommand.AnnounceLogViewLangId(e,a,t)elseif i then
local e=TppUiCommand.GetCurrentMissionSubGoalByNo(i)TppUiCommand.AnnounceLogViewLangId(e)end
end
function e.ShowColorAnnounceLog(o,t,i,n)if gvars.ini_isTitleMode then
return
end
local e=e.ANNOUNCE_LOG_TYPE[o]if e then
if n then
...
function e.ShowJoinAnnounceLog(i,t,a,o,n)if gvars.ini_isTitleMode then
return
end
local i=e.ANNOUNCE_LOG_TYPE[i]local e=e.ANNOUNCE_LOG_TYPE[t]if i and e then
if n then
TppUiCommand.AnnounceLogDelayTime(n)end
TppUiCommand.AnnounceLogViewJoinLangId(i,e,a,o)end
end
function e.ShowColorJoinAnnounceLog(n,t,a,o,i)if gvars.ini_isTitleMode then
return
end
local n=e.ANNOUNCE_LOG_TYPE[n]local e=e.ANNOUNCE_LOG_TYPE[t]if n and e then
if i then
TppUiCommand.AnnounceLogDelayTime(i)end
TppUiCommand.AnnounceLogViewJoinLangId(n,e,a,o,0,0,true)end
end
...
function e.GetMaxMissionTask(e)return 0
end
function e.EnableMissionTask(i,t)if t==nil then
t=true
end
if not n(i)then
return
end
local n={}for e,i in pairs(i)do
n[e]=i
end
local i=n.taskNo
if not i then
...
local o=e.GetTaskCompletedNumber(n)e.SetTaskLastCompleted(i,true)local i=e.GetTaskCompletedNumber(n)local n=e.GetMaxMissionTask(n)if n==nil then
return
end
if t then
if i>o then
e.ShowAnnounceLog("task_complete",i,n)end
end
TppMission.SetPlayRecordClearInfo()end
...
function e.SetTaskLastCompleted(i,n)local e=e.GetLastCompletedFlagIndex(i)if e then
...
function e._GetLastCompletedFlagIndex(n,e)local n=SsdMissionList.MISSION_ENUM[tostring(n)]if not n then
return
end
...
function e.GetTaskCompletedNumber(e)local n=SsdMissionList.MISSION_ENUM[tostring(e)]if not n then
return 0
end
local e=0
...
function e.ShowControlGuide(i)if not n(i)then
return
end
local t,o,n,s,a,_,d
t=i.actionName
...
local e=TppDefine.CONTROL_GUIDE[t]local i=TppDefine.CONTROL_GUIDE_LANG_ID_LIST[e]if i==nil then
return
end
if s then
...
TppUiCommand.SetButtonGuideDispTime(n)if not _ then
if(o==true)then
TppUiCommand.CallButtonGuideContinue(i,false,false,false)else
TppUiCommand.CallButtonGuide(i,false,false,false)end
else
local e=TppDefine.PAUSE_CONTROL_GUIDE[e]if e then
TppUiCommand.CallControllerHelp(e,i)end
end
...
function e.StartDisplayTimer(e)end
function e.ShowSavingIcon(e)TppUiCommand.CallSaveMessage(e)end
...
function e.PreloadLoadingTips(e)LoadingTipsMenuSystem.PrefetchBegin(e)end
function e.StartLoadingTips()local e=TppStory.GetCurrentStorySequence()if e<TppDefine.STORY_SEQUENCE.CLEARED_TUTORIAL then
LoadingTipsMenuSystem.SetBgImageType(LoadingTips.BG_2)else
LoadingTipsMenuSystem.SetBgImageType(LoadingTips.BG_1)end
TppUiCommand.StartLoadingTipsCommon()end
function e.FinishLoadingTips()TppUiCommand.RequestEndLoadingTips()end
function e.SetCoopFullUiLockType(e)e=e or TppStory.GetCurrentStorySequence()if Mission.IsEndCoopTutorialForStorySequence()then
SsdUiSystem.Unlock(SsdUiLockType.COOP_FULL)else
SsdUiSystem.Lock(SsdUiLockType.COOP_FULL)end
end
function e.OnAllocate(n)local i=true
if n.sequence then
...
MapInfoSystem.SetupInfos(TppQuestList)if n.sequence then
...
function e.Init()e.messageExecTable=Tpp.MakeMessageExecTable(e.Messages())local n=vars.missionCode
local i=TppMission.IsFreeMission(n)local n=TppMission.IsMultiPlayMission(n)TppUiStatusManager.UnsetStatus("MbEquipDevelop","NO_OPEN_SUPPORT_DIALOG")TppUiCommand.ResetCurrentMissionSubGoalNo()local i=(UiCommonDataManager.GetInstance()~=nil)if i then
if n then
e._UpdateCoopMissionPauseMenuList()else
e._UpdateSingleMissionPauseMenuList()end
local e=false
...
function e.EnableGameStatusOnFade()local e,i
...
if i then
e={}for i,n in pairs(i)do
e[i]=n
end
end
Tpp.SetGameStatus{target="all",enable=true,except=e,scriptName="TppUI.lua"}end
function e.SetDefaultGameOverMenu()local n=TppMission.IsMultiPlayMission(vars.missionCode)e._SetGameOverMenu(n)end
function e.SetGameOverMenu(e)if not n(e)or not next(e)then
return
end
GameOverMenuSystem.RegisterGameOverMenuItems(e)end
function e.SetDefenseGameMenu()if TppMission.IsMultiPlayMission(vars.missionCode)then
return
end
if n(mvars.ui_unsetPauseMenuSetting)then
table.insert(mvars.ui_unsetPauseMenuSetting,GamePauseMenu.RETURN_TO_BASE)else
mvars.ui_unsetPauseMenuSetting={GamePauseMenu.RETURN_TO_BASE}end
e._UpdateSingleMissionPauseMenuList()if n(mvars.ui_unsetGameOverMenuSetting)then
table.insert(mvars.ui_unsetGameOverMenuSetting,GameOverMenuType.RETURN_TO_BASE)else
mvars.ui_unsetGameOverMenuSetting={GameOverMenuType.RETURN_TO_BASE}end
e._SetGameOverMenu()end
function e.UnsetDefenseGameMenu()if TppMission.IsMultiPlayMission(vars.missionCode)then
return
end
local i=function(n,t)local e
...
if n(mvars.ui_unsetPauseMenuSetting)then
local e=i(mvars.ui_unsetPauseMenuSetting,GamePauseMenu.RETURN_TO_BASE)if e then
table.remove(mvars.ui_unsetPauseMenuSetting,e)end
end
e._UpdateSingleMissionPauseMenuList()if n(mvars.ui_unsetGameOverMenuSetting)then
local e=i(mvars.ui_unsetGameOverMenuSetting,GameOverMenuType.RETURN_TO_BASE)if e then
table.remove(mvars.ui_unsetGameOverMenuSetting,e)end
end
e._SetGameOverMenu()end
function e._UpdateCoopMissionPauseMenuList()local n=e._GetCoopMissionPauseMenuList()e._UpdatePauseMenu(n)end
function e._GetCoopMissionPauseMenuList()if Mission.EnableAbandonMissionForPauseMenu()then
return{GamePauseMenu.ABANDON_MATCH,GamePauseMenu.ONLINE_NEWS,GamePauseMenu.STORE_ITEM,GamePauseMenu.RECORDS_ITEM,GamePauseMenu.CONTROLS_AND_TIPS_ITEM,GamePauseMenu.OPEN_OPTION_MENU}else
return{GamePauseMenu.RETURN_TO_TITLE,GamePauseMenu.ONLINE_NEWS,GamePauseMenu.STORE_ITEM,GamePauseMenu.RECORDS_ITEM,GamePauseMenu.CONTROLS_AND_TIPS_ITEM,GamePauseMenu.OPEN_OPTION_MENU}end
end
function e._UpdateSingleMissionPauseMenuList()local n={GamePauseMenu.RESTART_FROM_CHECK_POINT,GamePauseMenu.RETURN_TO_BASE,GamePauseMenu.RETURN_TO_TITLE,GamePauseMenu.ONLINE_NEWS,GamePauseMenu.STORE_ITEM,GamePauseMenu.RECORDS_ITEM,GamePauseMenu.CONTROLS_AND_TIPS_ITEM,GamePauseMenu.OPEN_OPTION_MENU}e._UpdatePauseMenu(n)end
function e._UpdatePauseMenu(i)if not n(i)then
return
end
if Tpp.DEBUG_Where then
end
if mvars.ui_unsetPauseMenuSetting then
e._DeletePauseMenu(i,mvars.ui_unsetPauseMenuSetting)end
TppUiCommand.RegisterPauseMenuPage(i)end
function e._DeletePauseMenu(i,e)if not n(e)then
return
end
...
function e._SetGameOverMenu(t)local e={}local n=function(i,e)if not mvars.ui_unsetGameOverMenuSetting then
...
if not t then
n(e,GameOverMenuType.CONTINUE_FROM_CHECK_POINT)n(e,GameOverMenuType.RETURN_TO_BASE)end
GameOverMenuSystem.RegisterGameOverMenuItems(e)end
return e